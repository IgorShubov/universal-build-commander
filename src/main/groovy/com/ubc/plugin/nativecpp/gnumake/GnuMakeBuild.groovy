...
package com.ubc.plugin.nativecpp.gnumake
...
class GnuMakeBuild extends DefaultTask {
    ...
    void initParams() {
        def gnuMakeExt = project.extensions.getByName('gnumake')
        if (gnuMakeExt) {
            executable = gnuMakeExt.executable
            workingDir = gnuMakeExt.workingDir
            makefile = gnuMakeExt.makefile
            if (gnuMakeExt.makeTargets) {
                gnuMakeExt.makeTargets.split(" ").each {
                    targets.add(it)
                }
            }
        }
        println("Initial Params for GnuMakeBuild: executable = $executable, workingDir = $workingDir, makefile = $makefile, targets = $targets")
    }
    ...
    void exec() {
        ...
        buildCmdArgs()
        if (this.workingDir) {
            def gmakeWorkingDir = new File(workingDir)
            if (!gmakeWorkingDir.exists()) {
                gmakeWorkingDir.mkdirs()
            }
        }
